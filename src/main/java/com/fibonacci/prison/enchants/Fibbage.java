...
public class Fibbage extends CE {

	public Fibbage() {
		super("fibbage", "&dFibbage", 10, 300);
	}

	@Override
	public ItemButton getInfoButton() {
		return ItemButton.create(new ItemBuilder(Material.ENCHANTED_BOOK).setDisplayName(ColorUtils.colorize(this.getName() + " &d&lMaxLevel = " + this.getMaxLevel())).setLoreLinesFromArray("", ColorUtils.colorize("&eFibbage works with Explosion to"), ColorUtils.colorize("&egive you extra Fibs while you mine."), ColorUtils.colorize("&eHigher levels = more Fibs.")), (event) -> { ;
		});
	}
	...
	public ItemButton getBuyableButton(PrisonPlayer prisonPlayer, Player player, ItemButtonGUI gui) {
		return ItemButton.create(new ItemBuilder(Material.NETHER_STAR).setGlowing(true).setDisplayName(this.getName() + " &d&lMaxLevel = " + this.getMaxLevel()).setLoreLinesFromArray("", "&eYour Level - " + getPDCLevel(player.getInventory().getItemInMainHand()), "&dLevel " + (getPDCLevel(player.getInventory().getItemInMainHand()) + 1) + " Cost - " + NumberUtils.doubleToSuffixedNumber(getNextLevelPrice(player.getInventory().getItemInMainHand())) + " Fibs"), (event) -> {
			if (!ItemUtils.hasPDCEnchant(player.getInventory().getItemInMainHand(), CustomEnchants.EXPLOSION.getNamespace())) {
				player.sendMessage(ColorUtils.colorize("&7(&d&lEnchants&7) &cYou need to have Explosion Enchant to buy Fibbage."));
				return;
			}
			...
			ItemBuilder newPick = new ItemBuilder(player.getInventory().getItemInMainHand()).setGlowing(true).addCE(CustomEnchants.FIBBAGE, getPDCLevel(player.getInventory().getItemInMainHand()) + 1).addItemFlagsToItem(ItemFlag.HIDE_ENCHANTS, ItemFlag.HIDE_ATTRIBUTES);
